...
img1 = PhotoImage(file="images/player2.PNG")
can_widght.create_image(150, 420, image=img1)
img2 = PhotoImage(file="images/player4.png")
can_widght.create_image(1370, 420, image=img2)
...
l7 = Label(ws, text="UNO DECK", font="georgia 16 bold", fg="blue").place(x=1080, y=600)
...
def display():
    ...
    if playerTurn == 3:
        for i in range(len(players[playerTurn])):
            ...
            photos6.append(ImageTk.PhotoImage(image))
        ab = len(players[playerTurn])
        a = (790 - (74 * ab + 3 * (ab - 1))) / 2
        for i in range(len(players[playerTurn])):
            b = 1400
            btn = Button(ws, text=i + 1, image=photos6[i], borderwidth=0)
            btn.place(x=b, y=a)
            a = a + 77
    else:
        ...
        for i in range(len(players[3])):
            b = 1400
            l2 = Label(ws, text=i + 1, image=photos6[i], borderwidth=0)
            l2.place(x=b, y=a)
            a = a - 30
    var2 = discards[-1]
    ...
    Label(ws, image=photos5[0]).place(x=1070, y=380)
